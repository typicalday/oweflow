...
test('loadDefs discovers a workflow.yaml inside a subdirectory', () => {
  const dir = mkdtempSync(join(tmpdir(), 'owenwork-defs-sub-'));
  // a flat file...
  writeFileSync(
    join(dir, 'flat.yaml'),
    'name: flat\ninputs:\n  - name: x\nsteps:\n  - name: a\n    consumes: [x]\n    produces: [y]\n',
...
test('loadDefFile and loadDefs read YAML from disk', () => {
  const dir = mkdtempSync(join(tmpdir(), 'owenwork-defs-'));
  writeFileSync(
...
test('loadDefs: calls target must exist in resolver namespace', () => {
  const dir = mkdtempSync(join(tmpdir(), 'owenwork-defs-test-'));
  try {
    writeFileSync(
...
test('include cycle and calls cycle are detected independently', () => {
  ...
  const callsDir = mkdtempSync(join(tmpdir(), 'owenwork-defs-test-'));
  try {
    writeFileSync(
  ...
  const includeDir = mkdtempSync(join(tmpdir(), 'owenwork-defs-test-'));
  try {
    writeFileSync(
