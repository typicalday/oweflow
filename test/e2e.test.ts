/**
 * End-to-end tests: drive the real `bin/owenwork.mjs` binary as a subprocess,
...
function makeCli(db: string) {
  return (...args: string[]): any => {
    const r = raw(db, args);
    if (r.status !== 0) {
      throw new Error(`owenwork ${args.join(' ')} exited ${r.status}: ${r.stderr.trim()}`);
    }
...
function tmpDb(): string {
  const dir = mkdtempSync(join(tmpdir(), 'owenwork-e2e-'));
